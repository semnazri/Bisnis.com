...
@synthesize Label_canal=_Label_canal,table_canal=_table_canal,data_berita=_data_berita,b1,first_height,flag_tabs,loading,batas_atas;
...
- (void)viewDidLoad
{
    ...
    for (NSInteger a =0; a < [self.color_canal count]; a++) {
        ...
        //NSLog(@"JUML:AH DATA %@ isi %@ ",f_copy.color_id,[[Canal_Object Share_Manager]Canal_active_id]);
        if([f_copy.color_id isEqualToString:[[Canal_Object Share_Manager]Canal_active_id]]){
    ...
    self.data_berita = [NSMutableArray arrayWithObjects:nil];
    //cek front page
   ...
   // self.loading.center = CGPointMake(self.view.frame.size.width / 2, 200);
    //  [self.loading setTintColor:[UIColor blueColor]];
    // [self.loading setBackgroundColor:[UIColor blackColor]];
            ...
            if (latestLoans) {
                for (NSDictionary *loanDic in latestLoans) {
                    ...
                    if(self.index_day<1){
                        ...
                        if([self.flag_tabs isEqualToString:@"0"])
                        {
                            b2 = [[Frame_Berita alloc] initWhitId_berita2:[[loanDic objectForKey:@"post_id"] integerValue]p_judul_berita:[loanDic objectForKey:@"title"] p_tgl_berita:[loanDic objectForKey:@"post_date"] p_pukul:[NSString stringWithFormat:@" - %@ WIB",waktu] p_catagory:[loanDic objectForKey:@"category_id"]  p_datepost:[NSString stringWithFormat:@"%@ ",parent_category] p_img_berita:[NSString stringWithFormat:@"%@?w=250",[loanDic objectForKey:@"image_uri"] ] p_image_content:[loanDic objectForKey:@"is_live"]];
                        ...
                        if([self.flag_tabs isEqualToString:@"0"])
                        {
                        b2 = [[Frame_Berita alloc] initWhitId_berita2:[[loanDic objectForKey:@"post_id"] integerValue]p_judul_berita:[loanDic objectForKey:@"title"] p_tgl_berita:[loanDic objectForKey:@"post_date"] p_pukul:[NSString stringWithFormat:@"   %@ - %@ WIB" ,date,waktu] p_catagory:[loanDic objectForKey:@"category_id"]  p_datepost:[NSString stringWithFormat:@"%@ ",parent_category] p_img_berita:[NSString stringWithFormat:@"%@?w=250",[loanDic objectForKey:@"image_uri"] ] p_image_content:[loanDic objectForKey:@"is_live"]];
...
-(void)reloadDatasmoreindex:(NSString *)param{
    ...
    [NSURLConnection sendAsynchronousRequest:[[NSURLRequest alloc] initWithURL:[NSURL URLWithString:param]] queue:[[NSOperationQueue alloc] init] completionHandler:^(NSURLResponse *response, NSData *data, NSError *error) {
        ...
        if (!error) {
            NSArray *latestLoans = [self fetchData:data];
            if (latestLoans) {
                NSLog(@" ya begitu %lu",(unsigned long)[latestLoans count]);
    ...
    if([self.flag_tabs length]>3){
        if(indexPath.row==0){
            ...
            cell.label_judul_berita.font=self.font_size_judul_headline;
                 ...
                 cell= [tableView dequeueReusableCellWithIdentifier:@"cell_head4" forIndexPath:indexPath];
     ...
     //   cell.label_judul_berita.frame =CGRectMake(10,cell.label_judul_berita.frame.origin.y,cell.frame.size.width - 25,50);
        ...
        [cell.label_judul_berita setFrame:CGRectMake(CELL_CONTENT_MARGIN, 7, CELL_CONTENT_WIDTH - (CELL_CONTENT_MARGIN * 2),size.height)];
        ...
        cell.view_latest.frame = CGRectMake(0, cell.label_judul_berita.frame.size.height+18, cell.frame.size.width, 1.5);
...
//        });
        ...
        cell.label_tgl_berita.frame = CGRectMake(13+cell.label_canal_berita.frame.size.width,cell.label_tgl_berita.frame.origin.y, cell.frame.size.width, cell.label_canal_berita.frame.size.height);
...
-(CGFloat)tableView:(UITableView *)tableView estimatedHeightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if([self.flag_tabs length]>3){
        ...
        if(indexPath.row==0){
            ...
            size  = [text sizeWithFont:self.font_size_judul_headline constrainedToSize:constraint lineBreakMode:UILineBreakModeWordWrap];
            
        }else{
            
            size = [text sizeWithFont:self.font_size_judul constrainedToSize:constraint lineBreakMode:UILineBreakModeWordWrap];

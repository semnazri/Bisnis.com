...
@interface PagerDataViewControllerTerkait ()<GPPShareDelegate,GPPSignInDelegate>
{
...
@implementation PagerDataViewControllerTerkait
...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    NSArray *data= [[Terkait_Object sharedManager]array_terkait];
    ...
    [[NSMutableAttributedString alloc] initWithString:[self.dataLabel text]];    [s addAttribute:NSBackgroundColorAttributeName
...
-(IBAction)cmd_last:(id)sender{
    ...
    LiveViewController *monitorMenuViewController = [self.storyboard instantiateViewControllerWithIdentifier:@"PagerDataViewControllerLiveTerkait"];
...
-(void)reloadData:(NSMutableDictionary *)data_detail
{
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone) {
        ...
        if([[NSString stringWithFormat:@"%@",[data_detail objectForKey:@"is_series"]] integerValue]==1){
            ...
            //self.cell_series.frame=frame_hiden;
            self.cell_series.frame=CGRectMake(0, self.tinggi_judul, self.img_detail.frame.size.width,32);
...
//                }
                ...
                [self.web_detail loadHTMLString:[NSString stringWithFormat:@"%@ %@ %@",HTML,css_terkait,berita_terkait] baseURL:nil];
...
-(BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType{
    ...
    if ( navigationType == UIWebViewNavigationTypeLinkClicked ) {
        ...
        if ([request.URL.scheme isEqualToString:@"terkait"]) {
            [[Terkait_Object sharedManager]setPage_terkait:[NSString stringWithFormat:@"%li",(long)[request.URL.host integerValue]]];
            ...
            if ([request.URL.host isEqualToString:@"facebook"]) {
                ...
                if ([request.URL.host isEqualToString:@"twitter"]) {
                    SLComposeViewController *tweetSheet = [SLComposeViewController composeViewControllerForServiceType:SLServiceTypeTwitter];
                    [tweetSheet setInitialText:link_title];
            ...
            if ([[url absoluteString] rangeOfString:@"gmail"].location == NSNotFound) {
                ...
                LinkWebViewController *monitorMenuViewController = [self.storyboard instantiateViewControllerWithIdentifier:@"PagerDataViewControllerLinkTerkait"];
                [self presentViewController:monitorMenuViewController animated:UIModalTransitionStyleFlipHorizontal completion:nil];
    ...
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone) {
        ...
        switch ([[[arr_nav_info objectAtIndex:0] objectAtIndex:[self.dbManager.arrColumnNames indexOfObject:@"option_font"]] intValue]) {
            case 0:
                ...
                css =[NSString stringWithFormat:@"<style> .detail{font-size:22px; }</style>"];
                break;
            case 1:
                NSLog(@"3");
                ...
                css =[NSString stringWithFormat:@"<style> .detail{font-size:24px; }</style>"];
                break;
            case 2:
                NSLog(@"3");
                ...
                css =[NSString stringWithFormat:@"<style> .detail{font-size:28px; }</style>"];
                break;
            case 3:
                NSLog(@"3");
                ...
                css =[NSString stringWithFormat:@"<style> .detail{font-size:30px; }</style>"];
                break;

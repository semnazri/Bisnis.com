...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.type == HMSegmentedControlTypeText) {
        [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
            ...
            UILabel *welcomeLabel = [[UILabel alloc] initWithFrame:rect];
           // NSString *headertext=[NSString stringWithFormat:@"%@",[self attributedTitleAtIndex:idx]];

            welcomeLabel.text =[[self.sectionTitles objectAtIndex:idx] uppercaseString];
            
            
            if (self.selectedSegmentIndex == idx) {
                
                welcomeLabel.textColor = [UIColor whiteColor];
                welcomeLabel.shadowOffset = CGSizeMake(1, 0);
...
- (NSDictionary *)resultingTitleTextAttributes {
    NSDictionary *defaults = @{
        NSFontAttributeName : [UIFont fontWithName:@"STHeitiSC-Light" size:14.0f],
